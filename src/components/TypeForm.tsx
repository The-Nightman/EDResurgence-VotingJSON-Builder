 ...
 * @param {TypeObj} props.savedTypeForm - Optional prop, The saved variant data to pre-fill the form.
...
export const TypeForm = ({
  ...
  useEffect(() => {
    ...
    if (savedTypeForm) {
      setTypeFormData(savedTypeForm);
    }
  }, []);

  // useEffect runs when the modPack value changes
  useEffect(() => {
  ...
  const commands: { name: string; id: string }[] = [
    { name: "Sprint", id: "Server.Sprint" },
    { name: "Unlimited Sprint", id: "Server.UnlimitedSprint" },
    { name: "Assassinations", id: "Server.AssassinationEnabled" },
    { name: "Number Of Teams", id: "Server.NumberOfTeams" },
    { name: "Team Size", id: "Server.TeamSize" },
    { name: "Podium", id: "Server.PodiumEnabled" },
    { name: "Emotes", id: "Server.EmotesEnabled" },
    { name: "Podium Emotes", id: "Server.EmotesDuringPodiumEnabled" },
    { name: "Kill Command", id: "Server.KillCommandEnabled" },
    {
      name: "Kill Command During Podium",
      id: "Server.KillCommandDuringPodiumEnabled",
    },
    { name: "Near Victory Music", id: "Server.NearVictoryMusicEnabled" },
    { name: "Post Match Music", id: "Server.PostMatchMusicEnabled" },
  ];
      ...
      >
        ...
        <div className={`${formState.formCollapsed && "hidden"}`}>
            ...
            >
              <label className="flex flex-col min-w-48">
                ...
                <input
                  ...
                  value={typeFormData.randomChance}
                  min={0.1}
                  max={100}
                  step={0.1}
                />
          ...
          <fieldset className="flex flex-row flex-wrap gap-4 px-2">
            ...
            {/* Map the list of supported commands to generate form inputs and reduce bloat */}
            {commands.map((com) => {
              // if the command is team count or team size, generate their specific select layout
              if (
                com.id === "Server.NumberOfTeams" ||
                com.id === "Server.TeamSize"
              ) {
                return (
                  <Tooltip
                    key={`commands ${com.id}`}
                    // render the correct tooltip based on the command id
                    title={
                      com.id === "Server.TeamSize"
                        ? `Set the Team Size for the Server, this is the number of players 
                    ...
                    recommended to be set to 1 to distribute players evenly across teams.`
                        : `Set the ${com.name} for the Server`
                    }
                  >
                    <label className="flex flex-col min-w-24">
                      {/* if the command is NumberOfTeams render the shortened name */}
                      {com.name === "Number Of Teams" ? "Team Count" : com.name}
                      <select
                        name={com.name}
                        // render the correct accessibility description based on the command id
                        aria-description={
                          com.id === "Server.TeamSize"
                            ? `Set the Team Size for the Server, this is the number of players 
                        assigned to a team before being assigned to the next team in the list. This is 
                        ...
                        id={com.id}
                        // check to see if the command is already set in the commands array
                        // this is used for loading jsondata instead of making fresh json
                        // if the command is not found, set the value to -1
                        value={
                          (typeFormData.commands ?? [])
                            .find((commandStr: string) =>
                              commandStr.startsWith(com.id)
                            )
                            ?.split(" ")[1] ?? -1
                        }
                      ...
                      >
                        ...
                        {/* if the command is NumberOfTeams generate the correct options list */}
                        {com.id === "Server.NumberOfTeams"
                          ...
                          : // else generate the list of options for TeamSize
                            Array.from(
                              { length: 8 },
                              (_, index) => index + 1
                            ).map((num) => (
              ...
              // for all other commands generate generic
              else {
                return (
                  <Tooltip
                    key={`commands ${com.id}`}
                    title={`Set the ${com.name} option for the Server`}
                  >
                    <label className="flex flex-col min-w-24">
                      {/* if the command is KillCommandDuringPodiumEnabled render the shortened name */}
                      {com.name === "Kill Command During Podium"
                        ? "Podium Kill Command"
                        : com.name}
                      <select
                        name={com.name}
                        aria-description={`Set the ${com.name} option for the Server`}
                        id={com.id}
                        // check to see if the command is already set in the commands array
                        // this is used for loading jsondata instead of making fresh json
                        // if the command is not found, set the value to -1
                        ...
                        onChange={(e) => handleServerCommands(e, "commands")}
                      >
                        {/* if the command is Server.Sprint render its specific inherit option */}
                        <option value={-1}>None</option>
                        {com.id === "Server.Sprint" && (
            ...
            {commands.map((com) => {
              ...
              if (
                ...
                return (
                  <Tooltip
                    ...
                    title={
                      com.id === "Server.TeamSize"
                        ? `Set the Team Size for the Server after the match ends, this is the number of players 
                    assigned to a team before being assigned to the next team in the list. This is 
                    recommended to be set to 1 to distribute players evenly across teams.`
                        : `Set the ${com.name} for the Server after the match ends`
                    }
                  >
                    <label className="flex flex-col min-w-24">
                      {/* if the command is NumberOfTeams render the shortened name */}
                      ...
                      <select
                        ...
                        // if the command is not found, set the value to -1
                        value={
                          (typeFormData.endOfMatchCommands ?? [])
                            .find((commandStr: string) =>
                              commandStr.startsWith(com.id)
                            )
                            ?.split(" ")[1] ?? -1
              ...
              // for all other commands generate generic
                ...
                return (
                  <Tooltip
                    key={`commands ${com.id}`}
                    title={`Set the ${com.name} option for the Server after the match ends`}
                  >
                    <label className="flex flex-col min-w-24">
                      {/* if the command is KillCommandDuringPodiumEnabled render the shortened name */}
                      {com.name === "Kill Command During Podium"
                      ...
                      <select
                        name={com.name}
                        aria-description={`Set the ${com.name} option for the Server after the match ends`}
                        id={com.id}
                        // check to see if the command is already set in the commands array
                        // this is used for loading jsondata instead of making fresh json
                        // if the command is not found, set the value to -1
